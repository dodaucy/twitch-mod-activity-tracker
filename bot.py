...
class Cog(commands.Cog):
    def __init__(self, bot: commands.Bot, broadcaster):
        self.bot = bot
    ...
    async def help(
        ...
        if command is None:
            ...
            for command in LANGUAGE_STRUCTURE['commands']:
                if not config.enable_about_command:
                    if command == "about":
                        continue
            ...
            embed = disnake.Embed(
                ...
                color=config.embed.color.normal
            )
        else:
        ...
        await inter.response.send_message(
            embed=embed,
            ephemeral=config.embed.ephemeral
        )
    ...
    if config.enable_about_command:
